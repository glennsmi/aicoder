...
export const createInvitation = onCall({ region: 'europe-west2', secrets: [MAILER_SEND_KEY] }, async (request) => {
...
export const resendInvitation = onCall({ region: 'europe-west2', secrets: [MAILER_SEND_KEY] }, async (request) => {
  const { auth, data } = request
  if (!auth?.uid || !auth.token?.email) {
    throw new HttpsError('unauthenticated', 'User must be authenticated')
  }
